...
# 스테이지 만들기
stage = pygame.image.load(os.path.join(image_path, "stage.png")) # 스테이지 이미지를 가져옴.
stage_size = stage.get_rect().size # 스테이지 이미지의 크기를 구해옴
...
character_width = character_size[0] # 캐릭터의 가로크기 대괄호 안에 0을 넣으면 가로크기를 구한다.
character_height = character_size[1] # 캐릭터의 세로크기 대괄호 안에 1을 넣으면 세로크기를 구한다.
